...
def log_gaussian(
    ...
    Returns:
     - Value of the multivariate Gaussian, array of shape (N,)
    """
    K = µ.shape[0]
...
def log_t(
    ...
    Returns:
     - Value of the multivariate T distribution, array of shape (N,)
    """
    p = μ.shape[0]
...
class _TAGMFitData:
    ...
    def __init__(
        ...
        if y.shape[0] != X.shape[0]:
            raise ValueError(
                "Number of observations in X and y second dimension needs to"
                "be the same."
            )
        ...
        # I'm pretty sure this is wrong, but pRoloc has that. I think it
        # should be X.shape[0] instead.
        self.N = markerData.shape[0]
        ...
        if nu_0 is None:
            ...
            self.nu_0 = nu_0
        self.S_0 = np.diag(
                ((markerData - markerData.mean()) ** 2).sum(axis=0) / self.N
            ) / (self.K ** (1 / self.D))
        ...
        if eigenvalues.min() < np.finfo(float).eps:
            ...
            warnings.warn(
                "Co-linearity detected. Added a small multiple of the identity"
                "to the covariance."
            )
        ...
        for k in range(self.K):
            ...
            self.S_k[k] = (
                ...
                + (XX.T @ XX)
                + (self.lambda_0 *
                   (self.mu_0[:, np.newaxis] * self.mu_0[np.newaxis, :]))
                - lambda_k[k] * (self.mu_k[k, :, np.newaxis]
                                 * self.mu_k[k, np.newaxis, :])
            )
        nu_k = self.nu_0 + self.weights
        self.sigma_k = self.S_k / \
            (nu_k[:, np.newaxis, np.newaxis] + self.D + 1)
...
class TAGMMAP(ClassifierMixin, BaseEstimator):
    ...
    The TAGM-MAP classification model introduced to assign localisation terms
    and organelles to high-throughput proteomics data (LOPIT-DC, HyperLOPIT,
    etc.). This enhances a regular Bayesian Gaussian Mixture with an outlier
    component distributed according to a multivariate T distribution [1, 2].

    Parameters
    ----------

    n_components : int, default=300
        The number of mixture components. This is only used when no training
        data is given in `fit` or `fit_predict`.

    max_iter : int, default=100
        The maximum number of iterations allowed. Shows a warning if this is
        reached without the model having converged.

    tol : float, default=1e-4
        The convergence criterion. Convergence is reached the difference
        between log likelihood of the current and the previous EM iteration
        is less than `tol`.

    weight_concentration_prior : float, default=1.0
        The concentration of each component on the Dirichlet weight
        distribution (similar to sklearn.mixture.BayesianGaussianMixture).

    mean_precision_prior : float, default=0.01
        The precision prior on the mean Gaussian distribution (similar to
        sklearn.mixture.BayesianGaussianMixture).

    degrees_of_freedom_prior : float or None, default=None
        The prior of the number of degrees of freedom on the covariance
        distributions (similar to sklearn.mixture.BayesianGaussianMixture).
        If None, it's set to `n_features+2`

    outlier_conjugate_shape_prior : tuple of floats, default=(2, 10)
        The parameters of the conjugate Beta distribution describing the
        outlier component.

    outlier_degrees_of_freedom : float, default=4
        The degrees of freedom of the multivariate T distribution describing
    ...
    n_iter_ : int
        Number of EM steps required to reach convergence (if convergence was
        reached).
    ...
    .. [1] `Crook OM, Mulvey CM, Kirk PDW, et al. A Bayesian mixture modelling
            approach for spatial proteomics. PLoS Computational Biology
            14(11):e1006516 2018.
            <https://doi.org/10.1371/journal.pcbi.1006516>`

    .. [2] `Crook OM, Breckels LM, Lilley KS et al. A Bioconductor workflow
            for the Bayesian analysis of spatial proteomics. F1000Research
            2019, 8:446
    ...
    def _binarize(self, y, unknown_label):
        self.classes_ = np.array(
            [unknown_label] + list(unique_labels(y[y != unknown_label])))
    ...
    def fit(self, X, y=None, unknown_label=-1):
        ...
        Fits the model by iterating the expectation-maximisation (EM)
        algorithm until either convergence is achieved or `max_iter` is
        reached.
        ...
        y : None or array-like of shape (n_samples, n_outputs), default=None
            Training data, in the form of one row per data point given in `X`.
            Each column represents a different label. A data point for which
            the classification is known has at least one column that is `True`,
            but can have more than one. If None, the initial parameters are
            chosen by running a k-means clustering algorithm finding
            n_components clusters.

        unknown_label: str or int, default=-1
            Determines which label in `y` is treated as unknown. Common values
            are "unknown" or `-1`.
        ...
        for _ in range(self.max_iter):
            ...
            warnings.warn(
                "max_iter iterations reached and max likelihood convergence"
                "not achieved."
            )
    ...
    def _mstep(self):
        ...
        xm_k: NDArray[np.float64] = (
            (
                d.ab[:, :, 0, np.newaxis] * d.unknowns[np.newaxis, :, :]
            ).sum(axis=1) /
            d.ab[:, :, 0].sum(axis=1)[:, np.newaxis]
        )

        # The definitions of lambda_k and nu_k are different in pRoloc
        # compared to the paper. Not sure why, but we're going with pRoloc
        # here.
        a_k: NDArray[np.float64] = d.ab[:, :, 0].sum(axis=1)
        lambda_k = d.lambda_0 + d.weights
        nu_k = d.nu_0 + d.weights

        # Again, this uses lambda_k * d.m_k (the initial guess that isn't
        # updated), but according to the paper this should be
        # d.lambda_0 * d.mu_0 instead!
        ...
        # This /should/ be d.mu_0 according to the paer. The code in pRoloc
        # uses mk.
        ...
        d.sigma_k = (d.S_k + xixk) / (nu_k + a_k +
                                      d.D + 2)[:, np.newaxis, np.newaxis]
    ...
    def _eval_loglikelihood(self):
        ...
        Q += d.ab[:, :, 0].sum() * np.log(1 - d.epsilon) + \
            d.ab[:, :, 1].sum() * np.log(d.epsilon)
        Q += beta.logpdf(d.epsilon, d.u, d.v)
        Q += dirichlet.logpdf(d.pi_k, alpha=d.beta_0 / d.K)
        return Q

    def predict_proba(self, X: NDArray[np.float64]):
        """Evaluate the probability of each localisation of the samples in X.

        The first returned column indicates the probability that a data point
        is an outlier.
        ...
        probability : array of shape (n_samples, n_outputs + 1)
            Probability of a data point belonging to each localisation
            component. The first column contains the probability of a data
    ...
    def predict(self, X: NDArray[np.float64]):
        ...
        A label of `0` indicates that the data point was assigned to the
        outlier component.
    ...
    def fit_predict(self, X, y, return_assignment_probability=False,
                    return_outlier_probability=False, **kwargs):
        """Estimate model parameters and predict the localisation for X.

        Fits the model by iterating the expectation-maximisation (EM)
        algorithm until either convergence is achieved or `max_iter` is
        reached. After this, predicts the localisation of each data point
        by returning the localisation with the highest probability.

        A label of `0` indicates that the data point was assigned to the
        outlier component.

        Note, that this function is different from calling `fit(X, y)`
        followed by `predict`. This function won't assign a class for
        entries for which training data exists in `y`, instead returning
        the class given in `y`.
        ...
        return_outlier_probability : bool, default=False
            Return the probability of an entry to be part of the outlier
        ...
        assignment_probability (optional): array of shape (n_samples,)
            Predicted probability of the assigned label, only returned if
            `return_outlier_probability=True`.

        outlier_probability (optional): array of shape (n_samples,)
            Predicted probability of an entry being part of the outlier
            component, only returned if `return_outlier_probability=True`.
        """
    ...
    def fit_predict_proba(self, X, y, **kwargs):
        ...
        Fits the model by iterating the expectation-maximisation (EM) algorithm
        until either convergence is achieved or `max_iter` is reached. After
        this, returns the probability for each class.
        The first returned column indicates the probability that a data point
        is an outlier.

        Note, that this function is different from calling `fit(X, y)`
        followed by `predict_proba`. This function won't calculate any
        probabilities for entries for which training data exists in `y`,
        instead setting the probability for the corresponding class to one.
        ...
        proba[~have_training, :] = self.predict_proba(
            self._data.X[~have_training, :])
        proba[have_training,
              self._data.y[have_training].argmax(axis=1)+1] = 1.0
        return proba
